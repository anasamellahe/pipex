...
void	child_proccess1(t_file *cmd1, t_file *cmd2, int *fd, char **env)
{
	...
	if (cmd1->pid == 0)
	{
		...
		close(fd[1]);
		if (execve(cmd1->cmd, cmd1->param, env) == -1)
		{
...
void	child_proccess2(t_file *cmd1, t_file *cmd2, int *fd, char **env)
{
	...
	if (cmd2->pid == 0 && (cmd2->fd == -1 || cmd2->cmd_f == 1))
	{
		free_fun(cmd1, cmd2);
		exit(126);
	}
	if (cmd2->pid == 0)
	{
		...
		close(fd[0]);
		if (execve(cmd2->cmd, cmd2->param, env) == -1)
		{
...
void	pipex(t_file *cmd1, t_file *cmd2, char **env)
{
	...
	child_proccess1(cmd1, cmd2, fd, env);
	child_proccess2(cmd1, cmd2, fd, env);
	close(fd[0]);
	close(fd[1]);
	close(cmd1->fd);
